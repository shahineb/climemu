...
def compute_bias_significance(target_data, pred_data, qlevels, seasons, seasons_month, ar6, domains, domain_names):
            ...
            for domain_idx, (domain, domain_name) in enumerate(zip(domains, domain_names)):
                ...
                for region_idx, region in domain.regions.items():
                    ...
                    for season_idx, season in enumerate(seasons):
                        ...
                        pvalues = [quantile_test(target_data_jkl, q=q, p=p).pvalue for (q, p) in zip(q_emulator, qlevels)]
                        portrait_data[:, j, k, season_idx] = np.asarray(pvalues)
                        _ = pbar.update(1)
                    k += 1
...
# Compute significance of tail bias
portrait_data = compute_bias_significance(target_data, pred_data, qlevels, seasons, seasons_month, ar6, domains, domain_names)
...
def portrait_plot(fig, ax, seasonal_data, vmax, y_labels):
    ...
    coll = PatchCollection(patches, array=np.array(colors_flat),
                           cmap='Greys', norm=norm, edgecolor='grey', linewidth=0.2)
...
def create_bias_extremes_plot():
    ...
    for i in range(len(qlevels)):
        ...
        fig, ax = portrait_plot(fig, ax, portrait_data[i], 0.05, y_labels)
