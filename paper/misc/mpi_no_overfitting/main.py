...
def main():
    ...
    # Filter to train only on 1 member from historical
    indexmap = train_dataset.cmip6data.indexmap
    historical_index = train_dataset.cmip6data.experiments.index("historical")
    exclude_mask = (indexmap[:, 0] == historical_index) & (indexmap[:, 1] > 0)
    train_dataset.cmip6data.indexmap = indexmap[~exclude_mask]
