...
def denoising_single_loss(model, ctx_size, x, doy, σ, key):
    ...
    denoised_x̃ = model(x̃_with_ctx, doy, σ)
...
@eqx.filter_jit
def denoising_batch_loss_doy(model, ctx_size, schedule, x, doy, key):
    """
    ...
    batch_loss = L(x, doy, σ, keys).mean()
    return batch_loss


@eqx.filter_jit
def denoising_make_step_doy(model, ctx_size, schedule, x, doy, key, opt_state, opt_update):
    """
