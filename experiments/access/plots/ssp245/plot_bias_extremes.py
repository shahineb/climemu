...
from experiments.access.plots.ssp245.utils import VARIABLES, load_data
        ...
        for domain_idx, (domain, domain_name) in enumerate(zip(domains, domain_names)):
            ...
            for region_idx, region in domain.regions.items():
                ...
                for l, season in enumerate(seasons):
                    ...
                    target_data_jkl = target_data_jk.sel(month=seasons_month[season]).values.flatten()
                    pred_data_jkl = pred_data_jk.sel(month=seasons_month[season]).values.flatten()
...
# plt.show()
plt.savefig("experiments/access/plots/ssp245/files/extremes_bias.jpg", dpi=300, bbox_inches='tight')